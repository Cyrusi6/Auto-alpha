...
def _build_parser() -> argparse.ArgumentParser:
    ...
    for name in ["plan", "run", "resume", "merge", "report", "smoke"]:
        ...
        sp.add_argument("--research-readiness-decision-path")
        sp.add_argument("--require-alpha-factory-ready", action="store_true")
        sp.add_argument("--require-validation-ready", action="store_true")
        sp.add_argument("--candidate-budget", type=int)
        sp.add_argument("--feature-set-name", default="ashare_features_v2")
        sp.add_argument("--alpha-experiment-store-dir")
        sp.add_argument("--consolidated-factor-store-dir")
        sp.add_argument("--validation-campaign-store-dir")
        sp.add_argument("--source-candidate-pool-path")
        sp.add_argument("--max-candidates-per-shard", type=int)
        sp.add_argument("--factor-certification-campaign-dir")
        sp.add_argument("--factor-certification-queue-path")
        sp.add_argument("--portfolio-campaign-dir")
        sp.add_argument("--certified-factor-pool-path")
        sp.add_argument("--require-portfolio-ready", action="store_true")
