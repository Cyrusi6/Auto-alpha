...
## 2026-07-26 — Task 055-KR2 Replacement-Safe Lease 与非循环发布信任闭环

- 在精确基线 `df24308eadab07128b9efead884355247e58a382` 上，新增的两个不可 skip 回归稳定复现 gateway 在 credential 返回后仍进入 fake transport，以及 application 在锁替换后提交并复用 12-stage generation；定向命令为 2 failed。最终实现冻结 commit `e903f3765eda39449d975ea30f7c7ce796f4f5aa` 上两项均通过，gateway 计数为 credential=1、transport=0，stage commit=0、final pointer 不存在。
- 新 `ReplacementSafeLease` 全程持有 canonical parent fd 与 lease fd，身份绑定 256-bit nonce、xattr instance secret、sealed content digest、scope/root/attempt/owner 和递增 durable fence。所有 credential、post-intent、transport、receipt、stage commit、pointer publication 与 owner-safe release 边界都复验 fd/path/nonce/fence；旧 owner 被 fencing 后不能提交或 unlink 新 owner。
- 最终 production-shaped rehearsal hash 为 `2b4c875a891a18310362713c9542640a8ae5a8680a689956571891a71ea7cc2d`，独立验证 hash 为 `337753b9da3842f8ed8bb0f995d9dc89d94ede2ef8e8dd3033f4bd871744ce3b`。Positive/empty 的 primary 与 sibling 首轮均执行 12/12，primary resume 均复用 12/12；47 个 crash/recovery case 全部通过。每分支 net 100 与 all-in 100 均为 `causal_valuation_blocked`，未包装成模拟成功。
- Candidate authority/checkpoint/final seal/report/final verification/evidence hash 分别为 `171388ea...54ef4`、`459e90f3...4cb98`、`d66b9ea0...e9d60`、`1d6f20a6...0e7fa`、`7decfbc4...9fa65`、`7ef28651...78ec`；source root 为 `7b564e80...e7ba6`。注册 canonical artifacts strict schema 为 23/23、0 errors、0 warnings、0 unknown；额外 threat model 由原生 validator 通过。
- 实现级 focused 为 91 passed，完整 pytest 与 full local CI 均为 901 passed（2 个既有 risk-model warning），CI 60/60 commands passed；wheel/sdist 构建、clean venv wheel import、生产 wheel 边界扫描通过。测试 collection=901，skip/xfail/deselection 均为 0。
- 当前最高状态仅为 `task055kr2_candidate_ready_for_independent_audit_no_network_executed`。I→E→A 最终锚必须再以仓库外固定的精确 A OID/digest 和 I 中 verifier 做 clean-clone/tamper 验证；candidate self-check 不等于 independent review。旧 `df24308` READY evidence 仅通过新 supersession lineage标记 superseded/non-executable/authorization-ineligible，原文件未改写。
