...
## 2026-08-28 — Governed downstream data-processing seams

### Outcome

- Added a provider-neutral security identity/lifecycle owner. Controlled CNINFO text fixtures can produce non-authorizing code, name, listing and delisting event candidates with separate known/effective timing; interval derivation forbids current-master backfill, detects revision/conflict/code-collision failures, can avoid materializing daily rows, and publishes a replayable immutable interval artifact. Candidate parsing, event evidence, timeline derivation and interval validation all bind the complete implementation root. The full official event population, governed pre-span seeds and an independent Admission Verdict are still absent, so neither parser candidates nor interval artifacts can self-grant PIT eligibility.
- Added provider-neutral `index_daily_bars` v2, `trade_calendar` v2 and `daily_bars` v3 evidence paths. The disk-backed daily path consumes a validated identity-interval generation, uses `daily_bars_sqlite_resume_v3`, takes a nonblocking work-identity lock, and checkpoints consumed input bytes, projected rows, the static identity/calendar axis and joined coverage state. Its immutable output archives five source roles—provider rows, calendar rows, normalizer conflicts, identity intervals and the identity binding—then independently reconstructs the archived normalization root and replays those sources against canonical rows, validity and gaps. All three manifests now use exact top-level and nested schemas, strict bool/int separation and exact regular-file tree closure. The complete signed raw capture reference—capture public key, terminal envelopes and source proof—is deliberately not embedded yet, so standalone evidence remains technically blocked; no current-schema real full-lake index, daily-bars or trade-calendar generation has been published.
- Extended the CSI attachment parser to bounded legacy BIFF XLS through a pinned `xlrd` toolchain and isolated worker. The worker contract binds the canonical module name, module hash, `xlrd`/Python identity and resource limits, including the CLI `__main__` child-process boundary. The source verifier no longer executes historical source from Git: it recognizes the published acquisition only through its complete signed source identity, then uses the current semantic owner to replay contract/plan semantics, details ancestry, request population, raw/wire/durable evidence, normalized bytes and attachment bodies. A no-Git runtime published and independently verified `csindex_attachment_semantics_4c9baed8dd2e2bc08c65ecce`; its planner proof is honestly recorded as `signed_historical_exact_source_identity_verified`, not current planner re-derivation. Sheet-level exact cover found eight real attachments containing both supported and unsupported CSI300-bearing sheets. Those attachments fail atomically instead of leaking 388 provisional row shapes; the generation retained two non-authorizing candidates across 439 attachments/80 XLS/25 CSI-bearing sheets and passed signed-source/body deep replay. It remains non-admissible: 93 images, eight mixed-schema attachments, four XLS error-cell inputs, historical publication/effective-time, seed, event-chain and weight requirements are blocked, and no PIT membership authorization was created.
- Added a resumable CNINFO document-postprocess owner without changing the v5 acquisition implementation. Controlled closure fixtures prove sealed plan/closure replay, bounded SQLite inventory indexing, stable at-most-60,000-document reference shards, one-body-at-a-time consumption, symlink/concurrent-writer rejection and durable shard/checkpoint/manifest updates. The owner has not consumed a complete real strong closure because v5 stopped before publication.
- Added a bounded CNINFO corporate-action document parser with a content-addressed extractor contract binding CPython, stdlib HTML parsing, Poppler, its dynamic libraries and fixed locale/environment. Controlled PDF/HTML fixtures prove subprocess time/address/output/stderr limits, source body/size/lineage replay, strict record-before-effective-before-pay/list dates and monotonic proposal/approval/implementation/correction checks. Adjustment completeness remains separate from strict EventLedger completeness, projection consumes identity intervals directly, and `semantic_candidate_eligible` is always false. A full strong-closure semantic run, cross-document adjudication and independent coverage verdict remain outstanding.
- Preserved the acquisition/semantics boundary: no new seam can activate the Data Admission Profile, Alpha search, holdout, shadow, paper or live stages. The current verdict remains `0/11 admitted`; implementation readiness is not evidence sufficiency.
- Added unified-command contract regressions for CNINFO post-processing and market evidence. The five new top-level domain manifests deliberately remain outside the platform's shallow artifact-schema catalog for this milestone; their exact domain validators and content-addressed publishers remain authoritative, while catalog registration is a separate observability task and cannot grant Data Admission.
- Removed the unreferenced `validation.walk_forward.campaigns` compatibility re-export; callers already use the split campaign owners directly. This offsets the dedicated CNINFO post-processing owner and keeps the production source-file budget at 450 without adding another adapter layer.
- The CNINFO v5 service did not remain active: it stopped fail-closed at 2026-08-27 17:28:12 after 879 successful physical documents and 881 attempts. Request `58911612` returned HTTP 200 with declared `Content-Length=8,665,372` but only 2,621,440 captured bytes; it was neither WAF nor implementation-root drift. The signed 2.2 GiB partial activity remains immutable and is not reusable. Treating that exact premature EOF as retryable changes the implementation root, so before the later stop decision a complete v6 rerun would have required new human authorization; no further network call is now planned.
- On 2026-08-28 the operator chose to stop that future full-document acquisition rather than authorize a half-terabyte, multi-day v6 rerun. The three historical CNINFO transient units were stopped; no CNINFO network process remains active and no existing inventory or partial evidence was deleted. The full closure is now a `Deferred Full Document Closure`; a later event-scoped run must use a new profile/contract and independent coverage verdict.
...
- Committed and pushed v5 as `278f630`, then started transient service `auto-alpha-cninfo-document-year-shards-v5-20260827.service` at 2026-08-27 15:48:58 Asia/Shanghai. The service spent 47 minutes 51 seconds replaying signed selected and aggregate parents before opening activity `c7425780...`; the first 19 network attempts were all HTTP-200 positive, with zero retry/error, from 16:36:49 to 16:37:35. At the observed 2.42 seconds/document stable rate, the 2011 shard is projected near 2026-08-28 morning and all nine shards near 2026-09-06 to 2026-09-07, excluding future provider pauses or unusually expensive normalization.

## 2026-08-24 — Parallel data-evidence lanes while document capture is blocked

### Outcome

- Added a provider-neutral `index_daily_bars` evidence seam that independently replays the signed Baostock capture, freezes canonical benchmark fields plus row validity, checks the research calendar exact cover and binds the `benchmark_control` consumer. Real replay published deterministic generation `free_provider_market_data_evidence_1bdf100c4f66d8334454057a`: 1,945/1,945 expected index days, zero missing/extra/duplicate dates and 1,945 valid rows. Technical value/coverage evidence is verified, while formal admission remains false until the trade calendar, acquisition authority, provider origin/runtime isolation and Source Freeze consumer binding pass.
- Added signed-wire body replay and a bounded stdlib XLSX semantic parser for current CSI attachments. The 439-object real slice is now completely classified: seven XLSX files yielded 350 non-authorizing CSI300 change candidates, 259 files contain no CSI300 reference, 80 legacy XLS files and 93 images retain explicit unsupported-format blockers. Known-at, effective-at, attachment-to-announcement semantics, full event-chain coverage, the 2011 seed and historical weights remain unproved; no candidate row can become PIT membership.
- Added a provider-neutral PIT control-state derivation seam with separate `known_at` and `effective_at`, evidence-qualified pre-span seeds, conservative intraday restriction/resumption timing and deterministic conflict/unknown propagation. Missing or current-only seeds, unproved versions, unknown timing and conflicting transitions fail closed. A real Baostock reconciliation snapshot stays unknown because it lacks official event timing and an admissible pre-span seed; the seam always emits `data_admission_eligible=false` pending an independent verdict.
- Removed three unreferenced compatibility-only export modules (`scheduler.resources`, `simulator.ledger` and `certification.campaigns`) instead of increasing the repository source-file budget. Their owning implementations remain in `gpu_probe`, `ledger_run` and `campaigns_registry`; repository imports already use those owners directly. This keeps the 450-file architecture ceiling while adding the three governed data seams.
- Replayed the second CNINFO v4 pause from its signed raw envelope. The 2011 activity retained 3,659 positive documents and stopped after 3 hours 58 minutes on request `59086622`: HTTP 200, URL, MIME, Content-Length and SHA-256 passed, while a 20-byte fixed-width legacy binary record after a valid final xref caused the sole structural failure. `pdfinfo` and `pdftotext` both parse the complete document; stripping only that 20-byte record makes the existing validator pass. A bounded exact-shape validator is covered by a red/green regression and the real envelope, but its changed implementation root is a proposed v5 contract and has not reused the v4 authorization or restarted network acquisition.
- Pre-scanned the quarantined but byte-verifiable 8,129-document 2011 corpus as non-authorizing format fixtures: all 6,936 PDFs and 1,193 HTML documents pass the proposed gate, with no additional structural class discovered. This scan cannot repair weak source ancestry, but reduces the risk of another format-only v5 restart without reading research targets or opening a holdout.
- Kept the overall verdict at `0/11 admitted`. These parallel lanes reduce implementation and semantic uncertainty; they do not activate the Data Admission Profile, authorize Alpha search, or open holdout, shadow, paper or live stages.
...
- Started transient user service `auto-alpha-cninfo-document-year-shards-v4-20260822.service` at 2026-08-22 17:50:21 Asia/Shanghai. It is active on the 2011 shard and first replays all signed inventory/aggregate parents before opening the new v4 activity; no preflight work is reported as a downloaded document. The service serializes all nine years and exits on the first pause or nonzero shard outcome.
...
- Completed current-root CNINFO base discovery generation `cca3f9cd...`: 433/433 logical requests, 432 positive, one empty, no error and 13,113,875 response bytes. Transient user service `auto-alpha-cninfo-base-chain-20260820.service` has advanced to base inventory and binds it to that exact manifest. The changed discovery-parent content hash correctly creates activity `b5ab130e...`; older partial activity `cb84a4b1...` remains immutable and unused rather than being joined across ancestry.
- Added the governed `data free-cninfo-document-closure` command over the existing closure seam. It requires exactly one base and one supplemental signed inventory, defaults to the locked 2011–2019 request span, separates plan-only from explicit network authority, caps residual demand at 130,000 documents and retains the fixed 256 GiB response ceiling. Weak reusable documents still propagate weak ancestry; the command cannot wash the old 2011 capture into a strong closure.
...
- Completed immutable generation `free_provider_backfill_dcd7ba6e1337f498683a6650`: 1,946/1,946 logical requests positive, zero final errors, 1,947 attempts, 1,949 wire exchanges and 49,770,410 captured response bytes. Normalization produced 583,800 snapshot rows and zero conflicts. Independent current-identity replay verified the publication signature and reproduced normalized root `58ae0278...`; qualification remains `quarantined_reconciliation_only` because update date is not publication time and historical weights/provider-origin/runtime-isolation evidence remain absent.
...
- Real plan-only verification passed without network: 1,946 requests, population root `f171e595...`, request-plan hash `f448161d...` and request cap 11,676. The governed runtime then started the physical capture with locked `baostock==0.9.3` / client `00.9.30`, contract `d6e8a7a9...` and activity `a8e90319...`; these runtime-bound identities supersede the dependency-incomplete `.venv` preview identity. The run closed 1,621 dates as positive before `2018-08-30` exhausted exactly six connection attempts. All five approved cooldowns ran, the sixth error caused immutable pause `f46b57ae...`, no seventh request was made, and no generation was published. A later login probe succeeded, identifying a transient provider outage without authorizing another retry. The focused v2/legacy-allowlist tests are 9/9 and the complete official-backfill test file is 144/144.
