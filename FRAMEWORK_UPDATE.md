...
- 在精确基线 `df24308eadab07128b9efead884355247e58a382` 上，两个不可 skip 回归稳定复现 gateway 在 credential 返回后仍进入 fake transport，以及 application 在锁替换后提交并复用 12-stage generation；定向命令为 2 failed。最终 `I=96f25cf5d637a3eb16461b0e52b691b506f6a0e8` 上两项均通过，gateway 为 credential=1/transport=0，stage commit=0、final pointer 不存在。
...
- 最终 production-shaped rehearsal hash 为 `e7f3673a807c601bad9801b75c4f9e36bb3e086e963bcc7e8eed5af3e5423e3f`，独立验证 hash 为 `0cb57d56fd8327fb4335af28393e8c1cd5574d6ee1eca9eaff94dbc0ac1faf2c`。Positive/empty 的 primary 与 sibling 首轮均执行 12/12，primary resume 均复用 12/12；47 个 crash/recovery case 全部通过。每分支 net 100 与 all-in 100 均为 `causal_valuation_blocked`，未包装成模拟成功。
- Candidate authority/checkpoint/final seal/report/final verification/evidence hash 分别为 `cdaa2b26...32ab1`、`c7a9ee46...6154a`、`943f0e02...40eaa`、`4cd7f327...3d5bb`、`cc1fa542...48ba6`、`48d71ecf...44c3d`；source root 为 `1c716160...c89a1`。候选 evidence 使用内容寻址原子发布，旧固定路径 evidence 保持只读；authoritative verifier 与 release publisher 均从 rehearsal generation root 解析嵌套 catalog。注册 canonical artifacts strict schema 为 23/23、0 errors、0 warnings、0 unknown，threat model 由原生 validator 通过。
- 实现级 focused 为 94 passed；完整 pytest 与 full local CI 均为 904 passed（2 个既有 risk-model warning），CI 60/60 commands passed；wheel/sdist、Python 3.11 clean-venv wheel import 和生产 wheel 边界扫描通过。测试 collection=904，skip/xfail/deselection 均为 0。
