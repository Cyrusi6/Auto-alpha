...
- 在精确基线 `df24308eadab07128b9efead884355247e58a382` 上，两个不可 skip 回归稳定复现 gateway 在 credential 返回后仍进入 fake transport，以及 application 在锁替换后提交并复用 12-stage generation；定向命令为 2 failed。最终 `I=964b219a796a98784a23bb0f5f9e062830016d41` 上两项均通过，gateway 为 credential=1/transport=0，stage commit=0、final pointer 不存在。
...
- 最终 production-shaped rehearsal hash 为 `b65eaa091ff0acf606ace5001d83ee2fda8b4f9cea6ceee291ac409e5a3361f9`，独立验证 hash 为 `555c7582bb07009f3adbcd1d329d1b6356966bc602a9d4e95b8e476227fc0a66`。Positive/empty 的 primary 与 sibling 首轮均执行 12/12，primary resume 均复用 12/12；47 个 crash/recovery case 全部通过。每分支 net 100 与 all-in 100 均为 `causal_valuation_blocked`，未包装成模拟成功。
- Candidate authority/checkpoint/final seal/report/final verification/evidence hash 分别为 `fc452d90...93bb4b`、`253566f8...a6ae5`、`b3538644...c6b5`、`fc323593...25a47`、`b109353c...3ae45`、`319781e1...050f`；source root 为 `18c0310a...d944`。候选 evidence 改为内容寻址原子发布，旧固定路径 evidence 保持只读；注册 canonical artifacts strict schema 为 23/23、0 errors、0 warnings、0 unknown，threat model 由原生 validator 通过。
- 实现级 focused 为 93 passed；完整 pytest 与 full local CI 均为 903 passed（2 个既有 risk-model warning），CI 60/60 commands passed；wheel/sdist、Python 3.11 clean-venv wheel import 和生产 wheel 边界扫描通过。测试 collection=903，skip/xfail/deselection 均为 0。
