...
from auto_alpha.portfolio.simulation.backtest.run_backtest import apply_signal_lag
...
from auto_alpha.portfolio.risk.model.covariance import estimate_return_covariance
from auto_alpha.validation.lab.engine.materialization import FactorMaterializer, MaterializationInputs, load_materialized_factor
...
from auto_alpha.validation.lab.engine.policy import EngineeringRobustnessPolicy, load_validation_policy
from auto_alpha.validation.lab.campaigns.ingest import ingest_candidate_pool
from auto_alpha.validation.lab.campaigns.scheduler import run_validation_shards
...
def test_validation_scheduler_flag_creates_four_cuda_shards(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("auto_alpha.validation.lab.campaigns.scheduler.LocalComputeScheduler", FakeScheduler)
    strict = {}
    ...
    captured.clear()
    monkeypatch.setattr("auto_alpha.validation.lab.campaigns.scheduler.LocalComputeScheduler", AccumulatedFakeScheduler)
    result = run_validation_shards(
        store_dir,
        data_dir=str(tmp_path),
        factor_store_dir=str(tmp_path / "store"),
