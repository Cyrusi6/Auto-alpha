...
def test_dashboard_service_reads_risk_artifacts(tmp_path):
    ...
    (orders_plan / "auto_alpha.execution.trading.plan.json").write_text(
    ...
    (release_dir / "cli_inventory.json").write_text('{"entries":[{"module":"auto_alpha.data.ingestion.pipeline.run_pipeline"}]}', encoding="utf-8")
    ci_dir = tmp_path / "ci"
    ci_dir.mkdir()
    ...
    (corpus_dir / "auto_alpha.research.formulas.corpus.jsonl").write_text('{"formula_hash":"h1","formula_tokens":[0],"valid":true}\n', encoding="utf-8")
    ...
    assert service.load_cli_inventory()["entries"][0]["module"] == "auto_alpha.data.ingestion.pipeline.run_pipeline"
