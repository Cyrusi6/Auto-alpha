...
def test_formula_corpus_builds_sequences_preferences_and_schema_artifacts(tmp_path):
    ...
    corpus_path = tmp_path / "corpus" / "auto_alpha.research.formulas.corpus.jsonl"
    ...
    assert (tmp_path / "corpus" / "auto_alpha.research.formulas.corpus.jsonl.schema.json").exists()
...
def test_formula_corpus_merges_factor_store_records(tmp_path):
    ...
    row = json.loads((tmp_path / "corpus" / "auto_alpha.research.formulas.corpus.jsonl").read_text().splitlines()[0])
