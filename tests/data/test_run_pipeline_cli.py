...
from auto_alpha.data.ingestion.pipeline.ashare.dataset_registry import FULL_RESEARCH_DATASETS
from auto_alpha.data.ingestion.pipeline.ashare import LocalAshareStorage
from auto_alpha.data.ingestion.pipeline.ashare.providers.sample import SampleAShareDataProvider
...
def test_run_pipeline_sync_tushare_monkeypatched_provider_cannot_bypass_task055j(
    ...
    monkeypatch.setattr(
        "auto_alpha.data.ingestion.pipeline.ashare.manager.create_ashare_provider",
...
def test_run_pipeline_source_excludes_old_entrypoint_terms():
    source = Path("src/auto_alpha/data/ingestion/pipeline/run_pipeline.py").read_text(encoding="utf-8")
