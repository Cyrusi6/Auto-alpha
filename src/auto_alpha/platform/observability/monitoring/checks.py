...
from auto_alpha.execution.trading.paper import LocalPaperAccount, compute_account_performance
...
def check_task054b_engineering_baseline(report_path: str | Path | None) -> tuple[dict[str, Any], list[MonitoringAlert]]:
    payload = _read_json(Path(report_path)) if report_path else {}
    status = str(payload.get("status") or "missing")
    ...
    stage_verified = set(stages) == expected and all(bool(stages[name].get("verified")) for name in expected)
    queues_empty = all(int(payload.get(name, -1)) == 0 for name in (
        "certification_queue_count",
        "portfolio_queue_count",
        "paper_queue_count",
        "live_queue_count",
    ))
    downstream_blocked = all(payload.get(name) is False for name in (
        "certification_ready",
        "portfolio_ready",
        "paper_ready",
        "live_ready",
    ...
    alerts = []
    if payload and not verified:
        alerts.append(MonitoringAlert("error", "task054b_engineering_baseline", "Task 054-B production evidence is blocked", {"blocker_count": len(blockers)}))
    return {
        "exists": bool(payload),
        "status": status,
        "engineering_baseline_verified": verified,
        "verified_stage_count": sum(bool(row.get("verified")) for row in stages.values()),
        "engineering_blocker_count": len(blockers),
        "downstream_queues_empty": queues_empty,
        "certification_ready": False,
        "portfolio_ready": False,
        "paper_ready": False,
        "live_ready": False,
...
def check_task054c_engineering_baseline(report_path: str | Path | None) -> tuple[dict[str, Any], list[MonitoringAlert]]:
    payload = _read_json(Path(report_path)) if report_path else {}
    status = str(payload.get("status") or "missing")
    ...
    verified = (
        status == "task054c_engineering_baseline_completed_historical_selection_contaminated_certification_blocked"
    ...
    alerts = []
    if payload and not verified:
        alerts.append(MonitoringAlert("error", "task054c_engineering_baseline", "Task 054-C production evidence is blocked", {}))
    return {
        "exists": bool(payload),
        "status": status,
        "engineering_baseline_verified": verified,
        "historical_selection_contaminated": bool(payload.get("historical_selection_contaminated")),
        "downstream_queues_empty": queues_empty,
        "certification_ready": False,
        "portfolio_ready": False,
        "paper_ready": False,
...
def check_task055b_security_date_remediation(report_path: str | Path | None) -> tuple[dict[str, Any], list[MonitoringAlert]]:
    payload = _read_json(Path(report_path)) if report_path else {}
    status = str(payload.get("status") or "missing")
    readiness = payload.get("readiness") if isinstance(payload.get("readiness"), dict) else {}
    ...
    blocked = status == "task055b_security_date_evidence_remediation_blocked"
    valid = bool(payload) and (completed or blocked) and queues_empty and downstream_blocked
    alerts: list[MonitoringAlert] = []
    if payload and not valid:
        alerts.append(MonitoringAlert(
            "error",
            "task055b_security_date_remediation",
            "Task 055-B evidence boundary or downstream physical state is invalid",
            {"status": status, "queues": queues},
        ))
    return {
        "exists": bool(payload),
        "status": status,
        "security_date_evidence_closed": completed and valid,
        "remediation_blocked": blocked and valid,
        "factor_replay_ready": bool(readiness.get("factor_replay_ready")),
        "continuous_portfolio_valuation_ready": bool(readiness.get("continuous_portfolio_valuation_ready")),
        "future_research_data_ready": bool(readiness.get("future_research_data_ready")),
        "downstream_queues_physically_empty": queues_empty,
        "certification_ready": False,
        "portfolio_ready": False,
        "paper_ready": False,
        "live_ready": False,
...
def check_task055i_single_canary_authority(
    report_path: str | Path | None,
    ...
    if not report:
        return {
            "exists": False,
            "status": "",
            "task055i_boundary_valid": False,
            "single_canary_execution_ready": False,
            "operational_state_unproven": True,
            "certification_ready": False,
            "portfolio_ready": False,
            "paper_ready": False,
    ...
    verification = _read_json(Path(final_verification_path)) if final_verification_path else {}
    ready = "single_canary_execution_ready_no_network_executed"
    blocked = "task055i_single_canary_execution_blocked_no_network_executed"
    status = str(report.get("status") or "")
    ...
    try:
        offline = (
            ...
            and int(network.get("tushare_request_count") or 0) == 0
            and int(network.get("other_network_request_count") or 0) == 0
    ...
    authorization_bound = (
        ...
        and authorization.get("status") == status
        and authorization.get("resume_authorized") is False
        and authorization.get("canary") == canary
    )
    expected_verification = "passed" if status == ready else "blocked_verified"
    verification_bound = (
        verification.get("status") == expected_verification
        and verification.get("top_status") == status
        and verification.get("report_content_hash") == report.get("content_hash")
        and verification.get("execution_authorization_content_hash") == report.get("execution_authorization_content_hash")
        and verification.get("credential_read_count") == 0
        and verification.get("tushare_request_count") == 0
        and verification.get("other_network_request_count") == 0
        and verification.get("prospective_holdout_accessed") is False
    )
    ...
    common = (
        ...
        and report.get("operational_state_unproven") is True
    )
    canary_ready = common and status == ready and not blockers and readiness.get("single_canary_execution_ready") is True
    blocked_verified = common and status == blocked and bool(blockers) and readiness.get("single_canary_execution_ready") is False
    valid = canary_ready or blocked_verified
    details = {
        "exists": True,
        "status": status,
        "task055i_boundary_valid": valid,
        "offline_execution_proven": offline,
        "single_canary_execution_ready": canary_ready,
        "offline_blocked": blocked_verified,
        ...
        "tushare_request_count": int(network.get("tushare_request_count") or 0),
        "other_network_request_count": int(network.get("other_network_request_count") or 0),
    ...
    alerts = [] if valid else [
        MonitoringAlert(
            ...
            "Task 055-I single-canary execution authority or offline boundary is invalid",
            details,
        )
    ]
    return details, alerts
