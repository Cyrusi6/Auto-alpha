"""Local artifact reader for the A-share auto_alpha.platform.observability.dashboard."""
...
class AshareDashboardService:
    ...
    def load_execution_plan(self) -> dict[str, Any]:
        for path in self._execution_plan_candidates("auto_alpha.execution.trading.plan.json"):
            payload = self._read_json(path)
            if payload:
                return payload
        return {}
    ...
    def load_formula_corpus(self) -> pd.DataFrame:
        for path in self._formula_corpus_candidates("auto_alpha.research.formulas.corpus.jsonl"):
            frame = self._read_jsonl(path)
            if not frame.empty:
                return frame
        return pd.DataFrame()
