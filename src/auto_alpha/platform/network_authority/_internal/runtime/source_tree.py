...
from auto_alpha.platform.network_authority._internal.authorization.io import canonical_hash, publish_generation, sha256_file, validate_generation
...
def _source_entries(repository: Path) -> list[dict[str, Any]]:
    ...
    if not entries or not any(
        row["path"].startswith("auto_alpha/platform/network_authority/_internal/runtime/") for row in entries
    ):
