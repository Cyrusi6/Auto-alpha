...
from auto_alpha.portfolio.simulator.causal import build_valuation_surface, trace_causal_runs
...
from auto_alpha.portfolio.simulator.fee_projection import FeeProjectionCalculator
...
from auto_alpha.platform.governance.network.repair import (
    _assert_repair_in_matrix,
    _build_repaired_freeze,
    _materialize_exact20,
    _publish_raw_repair,
)
from auto_alpha.platform.governance.network.response_application import (
    _matrix_marks,
    _publish_dynamic_l2,
...
def production_stage_definitions() -> tuple[StageDefinition, ...]:
    ...
    return tuple(
        StageDefinition(
            ...
            validator_fqn=f"auto_alpha.platform.governance.network.application_components.{validators[name].__name__}",
        )
