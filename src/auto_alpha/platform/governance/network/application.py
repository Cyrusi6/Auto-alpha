...
from auto_alpha.platform.governance.network.response_application import _production_context
    ...
    for ordinal, row in enumerate(rows, start=1):
        if (
            ...
            or not str(row.get("validator_fqn") or "").startswith(
                "auto_alpha.platform.governance.network.application_components."
            )
        ):
