"""Local offline CI runner for repeatable release checks."""

from __future__ import annotations

import argparse
import importlib
import json
import sys
import tempfile
from datetime import datetime
from pathlib import Path

from auto_alpha.cli import normalize_python_module_command
from auto_alpha.platform.artifacts.schema.writer import write_json_artifact, write_jsonl_artifact
from auto_alpha.platform.governance.release.inventory import PLATFORM_MODULES

from .commands import CiCommandResult, run_command


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the local offline CI smoke suite.")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--quick", action="store_true")
    mode.add_argument("--full", action="store_true")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--skip-pytest", action="store_true")
    parser.add_argument("--skip-build", action="store_true")
    parser.add_argument("--pretty", action="store_true")
    parser.add_argument("--fail-fast", action="store_true")
    return parser


def main(argv: list[str] | None = None) -> int:
    args = _build_parser().parse_args(argv)
    root = Path.cwd()
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    mode = "full" if args.full else "quick"
    results: list[CiCommandResult] = []
    import_result = _import_smoke()
    if import_result is not None:
        results.append(import_result)
    if args.fail_fast and results and not results[-1].success:
        return _finish(args, output_dir, mode, results)
    quick_dir = output_dir / "quick_artifacts"
    smoke_dir = quick_dir / "sample_smoke"
    synthetic_root = Path(tempfile.mkdtemp(prefix="auto_alpha_ci_synthetic_", dir=output_dir))
    sample_data_dir = synthetic_root / "sample_data"
    post_download_ready_path = quick_dir / "post_download_ready_readiness.json"
    _write_post_download_ready_fixture(post_download_ready_path)
    _prepare_shadow_smoke_orders(quick_dir / "shadow_orders" / "plan")
    commands = [
        (
            "repository_layout_audit",
            [
                sys.executable,
                "-m",
                "dev_tools.repository_layout",
                "audit",
                "--root-dir",
                str(root),
            ],
        ),
        (
            "data_source_sample_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.quality.source_validation.run_smoke",
                "--provider",
                "sample",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(smoke_dir),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--datasets",
                "securities,trade_calendar,daily_bars,daily_basic,financial_features,daily_limits,adjustment_factors,index_members,corporate_actions",
                "--index-codes",
                "000300.SH",
                "--validate",
                "--stats",
                "--pretty",
            ],
        ),
        (
            "alpha_experiment_store_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.discovery.experiments.run_store",
                "smoke",
                "--output-dir",
                str(quick_dir / "alpha_experiment_store"),
                "--pretty",
            ],
        ),
        (
            "validation_campaign_store_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.validation.lab.campaigns.run_validation_store",
                "smoke",
                "--validation-campaign-store-dir",
                str(quick_dir / "validation_campaign_store"),
                "--output-dir",
                str(quick_dir / "validation_campaign_store"),
                "--pretty",
            ],
        ),
        (
            "factor_certification_campaign_store_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.validation.certification.campaigns.run_certification_campaign",
                "smoke",
                "--certification-campaign-store-dir",
                str(quick_dir / "factor_certification_campaign_store"),
                "--output-dir",
                str(quick_dir / "factor_certification_campaign_store"),
                "--pretty",
            ],
        ),
        (
            "portfolio_campaign_store_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.portfolio.construction.campaigns.run_portfolio_campaign",
                "smoke",
                "--portfolio-campaign-store-dir",
                str(quick_dir / "portfolio_campaign_store"),
                "--output-dir",
                str(quick_dir / "portfolio_campaign_store"),
                "--pretty",
            ],
        ),
        (
            "real_data_ops_sample_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.lake.operations.run_real_data",
                "run",
                "--profile-name",
                "sample_offline_small",
                "--provider",
                "sample",
                "--data-dir",
                str(quick_dir / "real_data_sample_data"),
                "--output-dir",
                str(quick_dir / "real_data_sample"),
                "--staging-dir",
                str(quick_dir / "real_data_sample_staging"),
                "--data-lake-registry-dir",
                str(quick_dir / "real_data_registry"),
                "--freeze-dir",
                str(quick_dir / "real_data_sample_freeze"),
                "--matrix-cache-dir",
                str(quick_dir / "real_data_sample_freeze" / "matrix_cache"),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--chunk-days",
                "2",
                "--mode",
                "append",
                "--validate",
                "--stats",
                "--build-matrix",
                "--refresh-matrix",
                "--pretty",
            ],
        ),
        (
            "backfill_observer_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.observer.run_observer",
                "smoke",
                "--run-dir",
                str(quick_dir / "observer_empty_run"),
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "backfill_observer"),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--expected-trade-days",
                "3",
                "--rate-limit-per-minute",
                "150",
                "--pretty",
            ],
        ),
        (
            "raw_data_landing_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.landing.run_landing",
                "smoke",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "raw_data_landing"),
                "--expected-trade-days",
                "3",
                "--pretty",
            ],
        ),
        (
            "raw_data_index_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.index.run_index",
                "smoke",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "raw_data_index"),
                "--datasets",
                "securities,trade_calendar,daily_bars,daily_basic",
                "--partition-granularity",
                "monthly",
                "--pretty",
            ],
        ),
        (
            "data_quality_lab_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.quality.lab.run_quality_lab",
                "smoke",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "data_quality_lab"),
                "--raw-data-index-manifest-path",
                str(quick_dir / "raw_data_index" / "raw_data_index_manifest.json"),
                "--use-raw-data-index",
                "--pretty",
            ],
        ),
        (
            "backfill_repair_dry_run_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.repair.run_repair",
                "dry-run",
                "--data-dir",
                str(sample_data_dir),
                "--run-dir",
                str(quick_dir / "observer_empty_run"),
                "--output-dir",
                str(quick_dir / "backfill_repair"),
                "--repair-plan-path",
                str(quick_dir / "backfill_observer" / "backfill_repair_plan.json"),
                "--pretty",
            ],
        ),
        (
            "research_data_readiness_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.pit.readiness.run_readiness",
                "assess",
                "--data-dir",
                str(sample_data_dir),
                "--raw-landing-report-path",
                str(quick_dir / "raw_data_landing" / "raw_data_landing_report.json"),
                "--freeze-readiness-path",
                str(quick_dir / "raw_data_landing" / "raw_freeze_readiness_decision.json"),
                "--output-dir",
                str(quick_dir / "research_data_readiness"),
                "--profile-name",
                "ci_quick",
                "--expected-start-date",
                "20240102",
                "--expected-end-date",
                "20240104",
                "--expected-trade-days",
                "3",
                "--pretty",
            ],
        ),
        (
            "post_download_execute_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.post_download.run_post_download",
                "run",
                "--data-dir",
                str(sample_data_dir),
                "--run-dir",
                str(quick_dir / "observer_empty_run"),
                "--staging-dir",
                str(quick_dir / "staging"),
                "--output-dir",
                str(synthetic_root / "post_download_execute"),
                "--registry-dir",
                str(synthetic_root / "registry"),
                "--freeze-dir",
                str(synthetic_root / "freeze"),
                "--matrix-cache-dir",
                str(synthetic_root / "matrix_cache"),
                "--readiness-report-path",
                str(post_download_ready_path),
                "--profile-name",
                "ci_quick",
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--execute",
                "--pretty",
            ],
        ),
        (
            "post_download_real_path_negative_smoke",
            [sys.executable, "-m", "auto_alpha.platform.governance.ci.post_download_negative_smoke", "--output-dir", str(quick_dir / "post_download_negative")],
        ),
        (
            "post_download_plan_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.ingestion.post_download.run_post_download",
                "plan",
                "--data-dir",
                str(sample_data_dir),
                "--run-dir",
                str(quick_dir / "observer_empty_run"),
                "--staging-dir",
                str(quick_dir / "staging"),
                "--output-dir",
                str(quick_dir / "post_download"),
                "--registry-dir",
                str(quick_dir / "registry"),
                "--freeze-dir",
                str(quick_dir / "freeze"),
                "--matrix-cache-dir",
                str(quick_dir / "matrix_cache"),
                "--readiness-report-path",
                str(quick_dir / "research_data_readiness" / "research_data_readiness_report.json"),
                "--profile-name",
                "ci_quick",
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--plan-only",
                "--pretty",
            ],
        ),
        (
            "real_data_ops_fake_tushare_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.lake.operations.run_real_data",
                "run",
                "--profile-name",
                "fake_tushare_small",
                "--provider",
                "tushare",
                "--fake-tushare-scenario",
                "success",
                "--data-dir",
                str(quick_dir / "real_data_fake_tushare_data"),
                "--output-dir",
                str(quick_dir / "real_data_fake_tushare"),
                "--staging-dir",
                str(quick_dir / "real_data_fake_tushare_staging"),
                "--data-lake-registry-dir",
                str(quick_dir / "real_data_fake_tushare_registry"),
                "--freeze-dir",
                str(quick_dir / "real_data_fake_tushare_freeze"),
                "--matrix-cache-dir",
                str(quick_dir / "real_data_fake_tushare_freeze" / "matrix_cache"),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--chunk-days",
                "2",
                "--mode",
                "append",
                "--validate",
                "--stats",
                "--build-matrix",
                "--pretty",
            ],
        ),
        (
            "matrix_refresh_validate_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.matrix.refresh.run_matrix_refresh",
                "refresh",
                "--data-dir",
                str(quick_dir / "real_data_sample_freeze" / "data"),
                "--data-freeze-dir",
                str(quick_dir / "real_data_sample_freeze"),
                "--data-version-manifest-path",
                str(quick_dir / "real_data_sample_freeze" / "dataset_version_manifest.json"),
                "--matrix-cache-dir",
                str(quick_dir / "real_data_sample_freeze" / "matrix_cache"),
                "--output-dir",
                str(quick_dir / "matrix_refresh"),
                "--refresh-mode",
                "skip_if_fresh",
                "--pretty",
            ],
        ),
        (
            "corporate_actions_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.pit.corporate_actions.run_actions",
                "report",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "corporate_actions"),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--reconcile-adjustment",
                "--pretty",
            ],
        ),
        (
            "paper_account_reset_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.trading.paper.run_account",
                "--account-dir",
                str(quick_dir / "account"),
                "reset",
                "--initial-cash",
                "1000000",
                "--pretty",
            ],
        ),
        (
            "broker_connectivity_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.connectivity.run_connectivity",
                "probe",
                "--profile-name",
                "mock_readonly",
                "--output-dir",
                str(quick_dir / "broker_connectivity"),
                "--connectivity-store-dir",
                str(quick_dir / "broker_connectivity_store"),
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "broker_readonly_mirror_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.mirror.run_readonly_mirror",
                "snapshot",
                "--connectivity-report-path",
                str(quick_dir / "broker_connectivity" / "broker_connectivity_report.json"),
                "--output-dir",
                str(quick_dir / "broker_readonly_mirror"),
                "--mirror-store-dir",
                str(quick_dir / "broker_readonly_mirror_store"),
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "broker_file_gateway_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.file_gateway.run_gateway",
                "smoke",
                "--gateway-store-dir",
                str(quick_dir / "broker_file_gateway"),
                "--output-dir",
                str(quick_dir / "broker_file_gateway"),
                "--outbox-dir",
                str(quick_dir / "broker_file_gateway" / "outbox"),
                "--inbox-dir",
                str(quick_dir / "broker_file_gateway" / "inbox"),
                "--trade-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "operator_handoff_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.handoff.run_handoff",
                "smoke",
                "--handoff-store-dir",
                str(quick_dir / "operator_handoff"),
                "--approval-store-dir",
                str(quick_dir / "approvals"),
                "--output-dir",
                str(quick_dir / "operator_handoff"),
                "--file-batch-id",
                "ci_file_batch_20240104",
                "--approval-id",
                "ci_order_approval_20240104",
                "--pretty",
            ],
        ),
        (
            "broker_mapping_certification_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.mapping.run_mapping_certify",
                "--output-dir",
                str(quick_dir / "broker_mapping_certification"),
                "--profile-name",
                "generic_broker_csv",
                "--policy",
                "dry_run_standard",
                "--pretty",
            ],
        ),
        (
            "production_orchestrator_plan_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.production.run_production",
                "plan-day",
                "--production-state-dir",
                str(quick_dir / "production_state"),
                "--output-dir",
                str(quick_dir / "production_plan"),
                "--run-mode",
                "shadow_only",
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--data-dir",
                str(sample_data_dir),
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--approval-store-dir",
                str(quick_dir / "approvals"),
                "--paper-account-dir",
                str(quick_dir / "account"),
                "--orders-dir",
                str(quick_dir / "shadow_orders"),
                "--pretty",
            ],
        ),
        (
            "production_replay_plan_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.replay.run_replay",
                "plan",
                "--replay-name",
                "ci_replay",
                "--replay-mode",
                "shadow_only",
                "--replay-state-dir",
                str(quick_dir / "production_replay_state"),
                "--output-dir",
                str(quick_dir / "production_replay"),
                "--start-date",
                "20240104",
                "--end-date",
                "20240104",
                "--trade-dates",
                "20240104",
                "--data-dir",
                str(sample_data_dir),
                "--pretty",
            ],
        ),
        (
            "shadow_lab_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.shadow_lab.run_shadow_lab",
                "smoke",
                "--replay-dir",
                str(quick_dir / "production_replay"),
                "--output-dir",
                str(quick_dir / "shadow_lab"),
                "--min-shadow-days",
                "0",
                "--pretty",
            ],
        ),
        (
            "live_readiness_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.platform.governance.readiness.run_readiness",
                "smoke",
                "--policy-profile",
                "sample_lenient_readiness",
                "--production-replay-report-path",
                str(quick_dir / "production_replay" / "production_replay_report.json"),
                "--shadow-lab-report-path",
                str(quick_dir / "shadow_lab" / "shadow_lab_report.json"),
                "--output-dir",
                str(quick_dir / "live_readiness"),
                "--pretty",
            ],
        ),
        (
            "broker_uat_lab_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.uat.run_uat",
                "run",
                "--output-dir",
                str(quick_dir / "broker_uat"),
                "--broker-store-dir",
                str(quick_dir / "broker_uat_store"),
                "--profile",
                "sample",
                "--adapter",
                "mock",
                "--connectivity-profile",
                "mock_readonly",
                "--connectivity-store-dir",
                str(quick_dir / "broker_uat_connectivity_store"),
                "--run-readonly-connectivity",
                "--run-readonly-mirror",
                "--run-file-roundtrip",
                "--run-kill-switch-test",
                "--pretty",
            ],
        ),
        (
            "program_trading_compliance_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.settlement.compliance.run_compliance",
                "build-pack",
                "--output-dir",
                str(quick_dir / "compliance"),
                "--artifact-dir",
                str(quick_dir),
                "--live-readiness-decision-path",
                str(quick_dir / "live_readiness" / "live_readiness_decision.json"),
                "--broker-file-gateway-report-path",
                str(quick_dir / "broker_file_gateway" / "broker_file_gateway_report.json"),
                "--operator-handoff-report-path",
                str(quick_dir / "operator_handoff" / "operator_handoff_report.json"),
                "--broker-mapping-certification-decision-path",
                str(quick_dir / "broker_mapping_certification" / "broker_mapping_certification_decision.json"),
                "--risk-control-report-path",
                str(quick_dir / "risk_controls" / "risk_control_report.json"),
                "--settlement-report-path",
                str(quick_dir / "settlement" / "settlement_report.json"),
                "--incident-report-path",
                str(quick_dir / "incidents" / "incident_report.json"),
                "--pretty",
            ],
        ),
        (
            "go_live_gate_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.go_live.run_go_live",
                "run",
                "--policy-profile",
                "sample_lenient_go_live",
                "--output-dir",
                str(quick_dir / "go_live_gate"),
                "--program-trading-compliance-pack-path",
                str(quick_dir / "compliance" / "program_trading_compliance_pack.json"),
                "--secret-scan-report-path",
                str(quick_dir / "compliance" / "secret_scan_report.json"),
                "--broker-uat-report-path",
                str(quick_dir / "broker_uat" / "broker_uat_report.json"),
                "--broker-adapter-contract-report-path",
                str(quick_dir / "broker_uat" / "broker_adapter_contract_report.json"),
                "--broker-file-gateway-report-path",
                str(quick_dir / "broker_file_gateway" / "broker_file_gateway_report.json"),
                "--operator-handoff-report-path",
                str(quick_dir / "operator_handoff" / "operator_handoff_report.json"),
                "--pretty",
            ],
        ),
        (
            "shadow_trading_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.trading.shadow.run_shadow",
                "smoke",
                "--production-run-id",
                "ci_shadow_20240104",
                "--orders-dir",
                str(quick_dir / "shadow_orders"),
                "--execution-plan-dir",
                str(quick_dir / "shadow_orders" / "plan"),
                "--output-dir",
                str(quick_dir / "shadow_trading"),
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "incident_response_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.operations.incidents.run_incident",
                "--incident-store-dir",
                str(quick_dir / "incidents"),
                "create",
                "--production-run-id",
                "ci_shadow_20240104",
                "--trade-date",
                "20240104",
                "--severity",
                "warning",
                "--code",
                "ci_smoke",
                "--title",
                "CI Smoke",
                "--description",
                "local ci incident smoke",
                "--pretty",
            ],
        ),
        (
            "settlement_engine_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.settlement.engine.run_settlement",
                "smoke",
                "--data-dir",
                str(sample_data_dir),
                "--account-dir",
                str(quick_dir / "account"),
                "--settlement-dir",
                str(quick_dir / "settlement"),
                "--as-of-date",
                "20240104",
                "--profile",
                "cn_ashare_paper_default",
                "--pretty",
            ],
        ),
        (
            "risk_controls_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.portfolio.risk.controls.run_controls",
                "smoke",
                "--output-dir",
                str(quick_dir / "risk_controls"),
                "--state-dir",
                str(quick_dir / "risk_state"),
                "--pretty",
            ],
        ),
        (
            "compute_cluster_probe",
            [
                sys.executable,
                "-m",
                "auto_alpha.platform.compute.scheduler.run_compute",
                "probe",
                "--state-dir",
                str(quick_dir / "compute_state"),
                "--output-dir",
                str(quick_dir / "compute_probe"),
                "--pretty",
            ],
        ),
        (
            "compute_cluster_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.platform.compute.scheduler.run_compute",
                "smoke",
                "--state-dir",
                str(quick_dir / "compute_state"),
                "--output-dir",
                str(quick_dir / "compute_smoke"),
                "--pretty",
            ],
        ),
        (
            "point_in_time_validate_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.data.pit.engine.run_pit",
                "validate",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "pit"),
                "--start-date",
                "20240102",
                "--end-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--min-listing-days",
                "0",
                "--pretty",
            ],
        ),
        (
            "model_core_register_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.formulas.runtime.engine",
                "--dry-run",
                "--register",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "model_out"),
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--report-dir",
                str(quick_dir / "reports"),
                "--pretty",
            ],
        ),
        (
            "feature_factory_v2_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.features.factory.run_features",
                "build",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "features_v2"),
                "--feature-set-name",
                "ashare_features_v2",
                "--pretty",
            ],
        ),
        (
            "feature_factory_v3_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.features.factory.run_features",
                "build",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "features_v3"),
                "--feature-set-name",
                "ashare_features_v3",
                "--pretty",
            ],
        ),
        (
            "feature_promotion_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.features.promotion.run_promotion",
                "smoke",
                "--data-dir",
                str(sample_data_dir),
                "--output-dir",
                str(quick_dir / "feature_promotion"),
                "--pretty",
            ],
        ),
        (
            "alpha_factory_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.discovery.factory.run_factory",
                "run",
                "--campaign-name",
                "local_ci_alpha",
                "--data-dir",
                str(sample_data_dir),
                "--factor-store-dir",
                str(quick_dir / "alpha_store"),
                "--output-dir",
                str(quick_dir / "alpha_factory"),
                "--report-dir",
                str(quick_dir / "alpha_reports"),
                "--feature-set-name",
                "ashare_features_v2",
                "--feature-set-manifest-path",
                str(quick_dir / "features_v2" / "feature_set_manifest.json"),
                "--candidate-budget",
                "12",
                "--template-budget",
                "4",
                "--random-budget",
                "4",
                "--mutation-budget",
                "2",
                "--crossover-budget",
                "1",
                "--corpus-budget",
                "0",
                "--proxy-max-candidates",
                "12",
                "--top-k",
                "4",
                "--pretty",
            ],
        ),
        (
            "leakage_audit_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.validation.firewall.leakage.run_audit",
                "--data-dir",
                str(sample_data_dir),
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--output-dir",
                str(quick_dir / "leakage"),
                "--as-of-date",
                "20240104",
                "--cutoff-date",
                "20240104",
                "--run-static-scan",
                "--run-truncation-test",
                "--max-formulas",
                "3",
                "--point-in-time",
                "--pretty",
            ],
        ),
        (
            "validation_lab_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.validation.lab.engine.run_validation",
                "run-suite",
                "--data-dir",
                str(sample_data_dir),
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--factor-id",
                "factor_ff245859feeec1e6",
                "--factor-type",
                "any",
                "--output-dir",
                str(quick_dir / "validation_lab"),
                "--run-placebo",
                "--placebo-trials",
                "2",
                "--run-regime",
                "--run-sensitivity",
                "--run-stress-backtest",
                "--pretty",
            ],
        ),
        (
            "factor_certification_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.validation.certification.factors.run_certify",
                "run",
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--factor-id",
                "factor_ff245859feeec1e6",
                "--output-dir",
                str(quick_dir / "factor_certification"),
                "--validation-lab-report-path",
                str(quick_dir / "validation_lab" / "validation_lab_report.json"),
                "--factor-validation-summary-path",
                str(quick_dir / "validation_lab" / "factor_validation_summary.json"),
                "--multiple-testing-report-path",
                str(quick_dir / "validation_lab" / "multiple_testing_report.json"),
                "--overfit-risk-report-path",
                str(quick_dir / "validation_lab" / "overfit_risk_report.json"),
                "--placebo-test-report-path",
                str(quick_dir / "validation_lab" / "placebo_test_report.json"),
                "--stress-backtest-report-path",
                str(quick_dir / "validation_lab" / "stress_backtest_report.json"),
                "--pretty",
            ],
        ),
        (
            "model_lifecycle_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.factors.lifecycle.run_lifecycle",
                "propose-activation",
                "--data-dir",
                str(sample_data_dir),
                "--factor-store-dir",
                str(quick_dir / "factor_store"),
                "--registry-dir",
                str(quick_dir / "model_registry"),
                "--approval-store-dir",
                str(quick_dir / "approvals"),
                "--output-dir",
                str(quick_dir / "model_lifecycle"),
                "--as-of-date",
                "20240104",
                "--create-review-package",
                "--require-approval",
                "--validation-lab-report-path",
                str(quick_dir / "validation_lab" / "validation_lab_report.json"),
                "--factor-validation-summary-path",
                str(quick_dir / "validation_lab" / "factor_validation_summary.json"),
                "--overfit-risk-report-path",
                str(quick_dir / "validation_lab" / "overfit_risk_report.json"),
                "--placebo-test-report-path",
                str(quick_dir / "validation_lab" / "placebo_test_report.json"),
                "--factor-certification-decision-path",
                str(quick_dir / "factor_certification" / "factor_certification_decision.json"),
                "--portfolio-lab-report-path",
                str(quick_dir / "portfolio_lab" / "portfolio_lab_report.json"),
                "--portfolio-robustness-report-path",
                str(quick_dir / "portfolio_lab" / "portfolio_robustness_report.json"),
                "--portfolio-certification-decision-path",
                str(quick_dir / "portfolio_certification" / "portfolio_certification_decision.json"),
                "--portfolio-certification-scorecard-path",
                str(quick_dir / "portfolio_certification" / "portfolio_certification_scorecard.json"),
                "--certified-portfolio-policy-path",
                str(quick_dir / "portfolio_certification" / "certified_portfolio_policy.json"),
                "--pretty",
            ],
        ),
        (
            "broker_statement_synthesize_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.statements.run_statement",
                "synthesize-from-internal",
                "--output-dir",
                str(quick_dir / "statement_source"),
                "--paper-account-dir",
                str(quick_dir / "account"),
                "--account-id",
                "paper_ashare",
                "--broker-name",
                "synthetic_broker",
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "broker_statement_import_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.broker.statements.run_statement",
                "import",
                "--source-dir",
                str(quick_dir / "statement_source"),
                "--output-dir",
                str(quick_dir / "statement_import"),
                "--account-id",
                "paper_ashare",
                "--broker-name",
                "synthetic_broker",
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "eod_reconciliation_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.execution.settlement.reconciliation.run_reconcile",
                "eod",
                "--statement-dir",
                str(quick_dir / "statement_import"),
                "--paper-account-dir",
                str(quick_dir / "account"),
                "--output-dir",
                str(quick_dir / "eod_reconciliation"),
                "--account-id",
                "paper_ashare",
                "--trade-date",
                "20240104",
                "--as-of-date",
                "20240104",
                "--pretty",
            ],
        ),
        (
            "artifact_schema_validate",
            [
                sys.executable,
                "-m",
                "auto_alpha.platform.artifacts.schema.run_validate",
                "--artifact-dir",
                str(smoke_dir),
                "--artifact-dir",
                str(quick_dir / "corporate_actions"),
                "--artifact-dir",
                str(quick_dir / "settlement"),
                "--artifact-dir",
                str(quick_dir / "compute_probe"),
                "--artifact-dir",
                str(quick_dir / "compute_state"),
                "--artifact-dir",
                str(quick_dir / "compute_smoke"),
                "--artifact-dir",
                str(quick_dir / "account"),
                "--artifact-dir",
                str(quick_dir / "pit"),
                "--artifact-dir",
                str(quick_dir / "leakage"),
                "--artifact-dir",
                str(quick_dir / "validation_lab"),
                "--artifact-dir",
                str(quick_dir / "validation_campaign_store"),
                "--artifact-dir",
                str(quick_dir / "factor_certification_campaign_store"),
                "--artifact-dir",
                str(quick_dir / "portfolio_campaign_store"),
                "--artifact-dir",
                str(quick_dir / "factor_certification"),
                "--artifact-dir",
                str(quick_dir / "portfolio_lab"),
                "--artifact-dir",
                str(quick_dir / "portfolio_certification"),
                "--artifact-dir",
                str(quick_dir / "model_registry"),
                "--artifact-dir",
                str(quick_dir / "model_lifecycle"),
                "--artifact-dir",
                str(quick_dir / "features_v2"),
                "--artifact-dir",
                str(quick_dir / "features_v3"),
                "--artifact-dir",
                str(quick_dir / "feature_promotion"),
                "--artifact-dir",
                str(quick_dir / "alpha_factory"),
                "--artifact-dir",
                str(quick_dir / "statement_import"),
                "--artifact-dir",
                str(quick_dir / "eod_reconciliation"),
                "--output-dir",
                str(output_dir / "schema"),
                "--write-manifest",
                "--pretty",
            ],
        ),
        (
            "formula_corpus_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.formulas.corpus.run_corpus",
                "--output-dir",
                str(quick_dir / "formula_corpus"),
                "--pretty",
            ],
        ),
        (
            "alphagpt_pretrain_smoke",
            [
                sys.executable,
                "-m",
                "auto_alpha.research.neural.search.run_pretrain",
                "--sequence-path",
                str(quick_dir / "formula_corpus" / "formula_sequences.jsonl"),
                "--output-dir",
                str(quick_dir / "pretrain"),
                "--epochs",
                "1",
                "--batch-size",
                "4",
                "--max-sequences",
                "8",
                "--device",
                "cpu",
                "--pretty",
            ],
        ),
        (
            "release_manager_dry_run",
            [
                sys.executable,
                "-m",
                "auto_alpha.platform.governance.release.run_release",
                "--release-name",
                f"local_ci_{mode}",
                "--output-dir",
                str(output_dir / "release"),
                "--artifact-dir",
                str(smoke_dir),
                "--run-import-smoke",
                "--run-dashboard-import",
                "--run-schema-validation",
                "--pretty",
            ],
        ),
    ]
    if mode == "full" and not args.skip_build:
        commands.append(
            (
                "package_build",
                [sys.executable, "-m", "hatchling", "build", "-d", str(output_dir / "dist")],
            )
        )
    if mode == "full" and not args.skip_pytest:
        commands.append(
            (
                "pytest",
                [
                    sys.executable,
                    "-m",
                    "pytest",
                    "--basetemp",
                    str(output_dir / "pytest_tmp"),
                ],
            )
        )
    commands = [
        (name, normalize_python_module_command(command))
        for name, command in commands
    ]
    for name, command in commands:
        result = run_command(name, command, root)
        results.append(result)
        if args.fail_fast and not result.success:
            break
    return _finish(args, output_dir, mode, results)


def _prepare_shadow_smoke_orders(plan_dir: Path) -> None:
    records = [
        {
            "parent_order_id": "ci_parent_000001",
            "child_order_id": "ci_child_000001_open",
            "trade_date": "20240104",
            "ts_code": "000001.SZ",
            "side": "BUY",
            "shares": 100,
            "order_value": 1000.0,
            "bucket": "open",
            "target_weight": 0.10,
            "price": 10.0,
        }
    ]
    write_jsonl_artifact(plan_dir / "child_orders.jsonl", records, artifact_type="child_orders", producer="ci")


def _write_post_download_ready_fixture(path: Path) -> None:
    payload = {
        "status": "raw_ready_for_freeze",
        "decision": {
            "status": "raw_ready_for_freeze",
            "core_ready": True,
            "expanded_ready": True,
            "alpha_ready": True,
            "matrix_ready": True,
            "validation_ready": True,
            "can_create_freeze": True,
            "can_build_matrix": True,
            "can_run_core_alpha_factory": True,
            "can_run_expanded_alpha_factory": True,
            "can_run_validation": True,
            "required_remediations": [],
            "warnings": [],
            "recommended_next_commands": [
                "uv run auto-alpha data post-download run --execute"
            ],
        },
        "summary": {
            "failed_job_count": 0,
            "quarantined_job_count": 0,
            "pending_job_count": 0,
            "weak_pit_dataset_count": 1,
            "unsafe_pit_dataset_count": 0,
            "data_size_gb": 0.0,
        },
        "dataset_checks": [],
    }
    write_json_artifact(path, payload, artifact_type="research_data_readiness_report", producer="ci")


def _finish(args: argparse.Namespace, output_dir: Path, mode: str, results: list[CiCommandResult]) -> int:
    payload = {
        "created_at": _utc_now(),
        "mode": mode,
        "status": "passed" if all(result.success for result in results) else "failed",
        "commands": [result.to_dict() for result in results],
    }
    write_json_artifact(output_dir / "ci_report.json", payload, artifact_type="ci_report", producer="ci")
    write_jsonl_artifact(output_dir / "ci_command_results.jsonl", [result.to_dict() for result in results], artifact_type="ci_command_results", producer="ci")
    (output_dir / "ci_report.md").write_text(_markdown(payload), encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2 if args.pretty else None, sort_keys=args.pretty))
    return 0 if payload["status"] == "passed" else 1


def _import_smoke() -> CiCommandResult:
    started = _utc_now()
    failures = []
    for module in [*PLATFORM_MODULES, "auto_alpha.platform.observability.dashboard.app"]:
        try:
            importlib.import_module(module)
        except Exception as exc:  # pragma: no cover - diagnostic path
            failures.append({"module": module, "error": str(exc)})
    payload = json.dumps({"failures": failures}, ensure_ascii=False)
    return CiCommandResult(
        name="import_smoke",
        command=["python", "-c", "import platform modules"],
        returncode=1 if failures else 0,
        duration_seconds=0.0,
        started_at=started,
        finished_at=_utc_now(),
        stdout_tail=payload,
        stderr_tail="",
    )


def _markdown(payload: dict) -> str:
    lines = [
        "# Local CI Report",
        "",
        f"- mode: `{payload.get('mode')}`",
        f"- status: `{payload.get('status')}`",
        "",
        "| command | status | duration | returncode |",
        "| --- | --- | ---: | ---: |",
    ]
    for result in payload.get("commands", []):
        lines.append(
            f"| {result.get('name')} | {result.get('success')} | {float(result.get('duration_seconds', 0.0)):.3f} | {result.get('returncode')} |"
        )
    return "\n".join(lines) + "\n"


def _utc_now() -> str:
    return datetime.utcnow().replace(microsecond=0).isoformat() + "Z"


if __name__ == "__main__":
    raise SystemExit(main())
