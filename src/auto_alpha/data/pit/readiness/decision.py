...
def _recommended_commands(status: str, has_core_blockers: bool, has_noncore_blockers: bool, summary: dict) -> list[str]:
    ...
    if has_core_blockers or has_noncore_blockers:
        commands.append("uv run auto-alpha data observe repair-plan --run-dir <run_dir> --data-dir <data_dir> --output-dir <reports_dir>")
    if status in {ResearchDataReadinessStatus.ready_for_freeze, ResearchDataReadinessStatus.ready_for_matrix, ResearchDataReadinessStatus.ready_for_alpha_factory}:
        commands.extend(
            [
                "uv run auto-alpha data land report --data-dir <data_dir> --output-dir <raw_landing_dir> --validate",
                "uv run auto-alpha data lake create-version --data-dir <data_dir> --registry-dir <registry_dir>",
                "uv run auto-alpha data lake create-freeze --data-dir <data_dir> --registry-dir <registry_dir> --freeze-dir <freeze_dir>",
                "uv run auto-alpha data matrix-refresh refresh --data-dir <freeze_data_dir> --matrix-cache-dir <matrix_cache_dir>",
            ]
        )
    if status == ResearchDataReadinessStatus.ready_for_alpha_factory:
        commands.append("uv run auto-alpha research alpha --data-dir <freeze_data_dir> --matrix-cache-dir <matrix_cache_dir> --pretty")
    if summary.get("matrix_freshness_status") != "fresh":
        commands.append("uv run auto-alpha data matrix-refresh plan --data-dir <data_dir> --matrix-cache-dir <matrix_cache_dir> --pretty")
    return commands
