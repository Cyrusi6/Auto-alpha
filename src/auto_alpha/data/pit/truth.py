"""Independent security-date truth reconstruction."""
...
def build_truth_v2(
    *,
    ...
    """Build the sole production truth generation through a sealed access broker."""

    intermediate = _build_source_truth(
    ...
    source = _validate_source_truth(intermediate["manifest_path"])
    ...
    return _publish_production_truth(
        Path(output_root),
        rows=rows,
        source=source,
        lineage={
...
def publish_truth_successor(
    *,
    ...
    """Publish a full-key immutable successor from one accepted exact response."""
    ...
    key = (
        str(request.get("ts_code") or params.get("ts_code") or ""),
        str(request.get("trade_date") or params.get("trade_date") or ""),
    )
    ...
    current = dict(rows[matches[0]])
    proof = {
        "api": "daily",
        "source_kind": "native_accepted_cache",
        "proof_quality": "validated_transport_receipt_and_v3_cache",
    ...
    current["daily_response_evidence"] = sorted(
        [row for row in evidence if row.get("proof_hash") != proof["proof_hash"]] + [proof],
        key=lambda row: str(row.get("proof_hash")),
    )
    if records:
        raw = dict(records[0])
        matrix_bar = {
        ...
        if current.get("corporate_action_validity") is False:
            ...
            state, reason = "TRADED_PRIMARY_BAR", "verified_exact_daily_bar"
        current.update(
            {
                "state": state,
                "reason_code": reason,
                ...
                "response_application": "positive_daily",
            }
        )
    else:
        current["response_application"] = "vendor_daily_absence_not_no_trade_proof"
        current["vendor_daily_absence"] = True
    current.pop("evidence_hash", None)
    current["evidence_hash"] = canonical_hash(current)
    rows[matches[0]] = current
...
def validate_truth_v2(path: str | Path) -> dict[str, Any]:
    manifest_path = _resolve_production_manifest(path)
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("schema_version") != PRODUCTION_TRUTH_SCHEMA or manifest.get("status") != "published":
        raise TruthV2Error("production_truth_schema_or_status_invalid")
    ...
    if canonical_hash(semantic) != manifest.get("content_hash"):
        raise TruthV2Error("production_truth_content_hash_mismatch")
    rows_entry = manifest.get("partitions", {}).get("rows") or {}
    rows_path = manifest_path.parent / str(rows_entry.get("path") or "")
    if not rows_path.is_file() or sha256_file(rows_path) != rows_entry.get("sha256"):
        raise TruthV2Error("production_truth_rows_partition_mismatch")
    ...
    if len(rows) != manifest.get("record_count") or len(keys) != len(set(keys)):
        raise TruthV2Error("production_truth_key_count_invalid")
    if canonical_hash(sorted(keys)) != manifest.get("key_root"):
        raise TruthV2Error("production_truth_key_root_mismatch")
    counts = dict(sorted(Counter(row["state"] for row in rows).items()))
    if counts != manifest.get("state_counts"):
        raise TruthV2Error("production_truth_state_counts_mismatch")
...
def _validate_source_truth(path: str | Path) -> dict[str, Any]:
...
def _load_inventory(reader: AccessBroker, manifest_path: str | Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
...
def _load_daily_proofs(
    reader: AccessBroker, manifest_path: str | Path
    ...
    try:
        ...
        write_artifact_sidecar(
            rows_path,
            {
                ...
                "schema_version": "1.0",
                "producer": "auto_alpha.data.pit.truth",
...
def _publish_production_truth(
    ...
    *,
    ...
    staging = Path(tempfile.mkdtemp(prefix=".truth.production.", dir=root))
    try:
        rows_path = staging / "truth_v2_rows.jsonl"
        rows_path.write_text(
            "".join(
        ...
        partition = _partition(rows_path)
        semantic = {
            "schema_version": PRODUCTION_TRUTH_SCHEMA,
            "status": "published",
            "review_version": "complete_bar_resume_and_lifecycle_precedence_v1",
            "max_date": PRODUCTION_MAX_DATE,
            "record_count": len(rows),
...
def _resolve_production_manifest(path: str | Path) -> Path:
    value = Path(path)
    if value.is_file():
        return value
    pointer = value / "current.json"
    ...
    if candidate.is_file():
        return candidate
    raise TruthV2Error("production_truth_manifest_missing")
