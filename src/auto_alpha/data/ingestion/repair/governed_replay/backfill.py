...
from auto_alpha.data.ingestion.pipeline.ashare.cache import TushareResponseCache, tushare_cache_source_hash
...
from auto_alpha.data.ingestion.pipeline.ashare.request_normalization import normalize_tushare_request, stable_json_hash, tushare_code_semantic_hash
...
def run_governed_backfill(config: GovernedBackfillConfig) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "schema_version": "2.0",
        "producer": "auto_alpha.data.ingestion.repair.governed_replay.backfill",
...
def _source_generation_code_hash() -> str:
    return semantic_source_hash(
        (
            "auto_alpha.data.ingestion.repair.governed_replay.backfill",
            "auto_alpha.data.ingestion.pipeline.ashare.cache",
            "auto_alpha.data.ingestion.pipeline.ashare.providers.tushare_client",
            "auto_alpha.data.ingestion.pipeline.ashare.request_normalization",
            "auto_alpha.data.ingestion.pipeline.ashare.dataset_registry",
        )
    )
