...
from auto_alpha.data.lake.store.freeze import write_freeze_validation_report, validate_freeze
from auto_alpha.data.lake.store.models import DatasetVersionRecord
from auto_alpha.data.lake.store.report import write_data_lake_report, write_dataset_version_manifest, write_research_freeze
from auto_alpha.data.ingestion.pipeline.ashare.config import AShareDataConfig
from auto_alpha.data.matrix.refresh.refresh import run_matrix_refresh
from auto_alpha.data.matrix.store.builder import build_matrix_cache
...
def run_real_data_pipeline(
    *,
    ...
    runbook = build_runbook(
        ...
        token_expiry=effective_profile.metadata.get("token_expiry"),
        resume_command=["python", "-m", "auto_alpha.data.lake.operations.run_real_data", "resume", "--output-dir", str(root), "--data-dir", str(data_dir)],
    )
