...
from .index_models import RawDataIndexStatus
from .index_registry import default_datasets
from .index_report import dumps, write_raw_data_index_artifacts
from .index_scanner import active_run_safety_check, build_raw_data_index
    ...
    if args.command == "plan" or args.plan_only:
        ...
        payload = {
            ...
            "commands": {
                ...
                "validate": f"uv run auto-alpha data index validate --data-dir {data_dir} --output-dir {output_dir} --pretty",
            },
        }
...
def _build_command(args: argparse.Namespace, data_dir: Path, datasets: list[str]) -> str:
    parts = [
        "uv run auto-alpha data index build",
        f"--data-dir {data_dir}",
