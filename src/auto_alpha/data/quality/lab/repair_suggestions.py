...
def _command_hint(dataset: str, action: str) -> str:
    if action == "compact_dedup":
        return f"uv run auto-alpha data sync --data-dir <stable_data_dir> --compact --datasets {dataset} --pretty"
    if action == "require_pit_review":
        return f"uv run auto-alpha data pit validate --data-dir <stable_data_dir> --datasets {dataset} --pretty"
    if action == "exclude_dataset_from_feature_family":
        return f"uv run auto-alpha research promote-features init-policy --denied-features-from-dataset {dataset} --pretty"
    if action == "inspect_empty_response":
        return f"uv run auto-alpha data validate --provider tushare --datasets {dataset} --allow-network --max-requests 5 --pretty"
    return f"uv run auto-alpha data repair plan --data-dir <stable_data_dir> --datasets {dataset} --pretty"
