    ...
    for record in records:
        ...
        for field in ["ann_date", "ex_date", "record_date", "pay_date"]:
            ...
            if value and not is_valid_date(value):
                collector.add("auto_alpha.data.pit.corporate_actions.date_fields", "corporate_actions", f"invalid {field}", key=ts_code, field=field, sample=record)
        if not record.get("ann_date"):
            collector.add("auto_alpha.data.pit.corporate_actions.date_fields", "corporate_actions", "missing ann_date", key=ts_code, field="ann_date", sample=record)
        for field in ["cash_div", "stk_div", "stock_div", "bonus_share"]:
            value = as_float(record.get(field))
            if value is not None and value < 0:
                collector.add("auto_alpha.data.pit.corporate_actions.non_negative", "corporate_actions", f"{field} is negative", key=ts_code, field=field, sample=record)
