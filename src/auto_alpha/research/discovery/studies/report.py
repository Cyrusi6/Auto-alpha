...
from auto_alpha.research.factors.engine.correlation import pairwise_correlation_table
...
def _render_markdown(result: BatchResearchResult) -> str:
    ...
    lines.extend(
        [
            ...
            "## Counts",
            "",
            f"- validation candidates: `{len(result.approved_factor_ids)}`",
            f"- research rejected: `{len(result.rejected_factor_ids)}`",
