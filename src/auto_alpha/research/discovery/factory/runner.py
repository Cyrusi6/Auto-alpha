...
class AlphaFactoryRunner:
    ...
    def _register_experiment_store(self, campaign, report_json: Path) -> None:
        ...
        from auto_alpha.research.discovery.experiments.comparison import compare_experiment_stores
    ...
    def _proxy_reference_matrices(self, loader) -> tuple[list[Any], str]:
        ...
        if not self.config.production_research:
            from auto_alpha.research.factors.engine.correlation import load_existing_factor_matrices
        ...
        from auto_alpha.validation.lab.engine.materialization import load_materialized_factor
        ...
        for shard_id in range(shard_count):
            ...
            jobs.append(
                ComputeJobSpec(
                    ...
                    command=[sys.executable, "-m", "auto_alpha.research.formulas.batch.run_batch_eval"],
                    args=args,
                    cwd=str(Path.cwd()),
