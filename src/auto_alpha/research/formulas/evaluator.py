"""Strict formula batch evaluation, sharding, merge, lineage, and command workflow."""
...
def merge_shard_outputs(shard_dirs: list[str | Path], output_dir: str | Path) -> dict:
    ...
    for shard_dir in shard_dirs:
        ...
        if not path.exists():
            missing_count += 1
            continue
        for row in _batch_merge_read_jsonl(path):
...
def _batch_merge_read_jsonl(path: Path) -> list[dict]:
...
from auto_alpha.platform.artifacts.schema.writer import write_json_artifact, write_jsonl_artifact
from auto_alpha.research.search.evaluation import build_factor_report, split_trade_dates, write_factor_report
from auto_alpha.research.factors.engine import FactorGateConfig, FactorResearchPipeline
from auto_alpha.research.factors.engine import preprocess_factor_with_validity
from auto_alpha.research.factors.store import (
    ExperimentRecord,
    FactorRecord,
    LocalFactorStore,
    make_experiment_id,
...
class FormulaBatchEvaluator:
    ...
    def _load_cache(self) -> dict[str, dict[str, Any]]:
        ...
        if not path.exists():
            return cache
        for row in _batch_evaluator_read_jsonl(path):
...
def requests_from_corpus(path: str | Path, max_records: int | None = None) -> list[FormulaEvalRequest]:
    requests = []
    for idx, payload in enumerate(_batch_evaluator_read_jsonl(Path(path))):
...
def _batch_evaluator_read_jsonl(path: Path) -> list[dict[str, Any]]:
    if not path.exists():
        return []
    records = []
...
def _load_requests(args: argparse.Namespace, parser: argparse.ArgumentParser) -> tuple[list, str | None]:
    from auto_alpha.research.formulas.candidates import default_candidates, load_candidates_json
