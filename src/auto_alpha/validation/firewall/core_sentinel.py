"""Deterministic in-process research-firewall sentinel."""
...
REQUIRED_SENTINEL_PATHS = ("raw_local", "raw_scheduler", "matrix_local", "matrix_scheduler")
...
class FirewallSentinelDataset:
    ...
    def validate(self) -> None:
        if self.feature_values.shape != self.feature_validity.shape or self.feature_values.ndim != 3:
        ...
        if len(self.trade_dates) != expected[1] or not self.formula_tokens:
            raise ValueError("sentinel date axis or formula invalid")
...
def run_research_firewall_sentinel(
    ...
    *,
    ...
    baseline = {
        ...
        for name, (dataset, execution) in path_inputs.items()
    }
    post_inputs = {
        name: (_mutate_dataset(dataset, firewall, after_cutoff=True), execution)
        for name, (dataset, execution) in path_inputs.items()
    }
    inside_inputs = {
        name: (_mutate_dataset(dataset, firewall, after_cutoff=False), execution)
        for name, (dataset, execution) in path_inputs.items()
    }
    post_cutoff = {
        name: _evaluate_path(dataset, firewall, vocab=vocab, execution=execution)
        for name, (dataset, execution) in post_inputs.items()
    }
    inside_cutoff = {
        name: _evaluate_path(dataset, firewall, vocab=vocab, execution=execution)
        for name, (dataset, execution) in inside_inputs.items()
    }
    ...
    consistent = len({baseline[name]["result_hash"] for name in REQUIRED_SENTINEL_PATHS}) == 1
    violations = [row for row in firewall.access_audit if not row.get("allowed", False)]
    blockers = _blockers(post_changes, diagnostic_changes, inside_misses, consistent, violations)
    payload = {
        "artifact_type": "research_firewall_sentinel",
        ...
        "proof": {
            "post_cutoff_research_change_count": int(post_changes),
            "diagnostic_change_count": int(diagnostic_changes),
            "inside_cutoff_cache_miss_count": int(inside_misses),
            "raw_matrix_local_scheduler_consistent": consistent,
            "access_violation_count": len(violations),
        },
    ...
    payload = attach_artifact_metadata(payload, "research_firewall_sentinel", "research_firewall")
    target = Path(output_dir)
    target.mkdir(parents=True, exist_ok=True)
    _atomic_json(target / "research_firewall_sentinel.json", payload)
    return payload


def _evaluate_path(dataset: FirewallSentinelDataset, firewall: DateFirewall, *, vocab, execution: str) -> dict[str, Any]:
    ...
    target_validity = dataset.target_available.index_select(1, _index(research_indices, dataset.target.device))
    ...
    executed = StackVM(vocab).execute_with_validity(list(dataset.formula_tokens), research_values, research_validity)
    if executed is None:
        raise RuntimeError("sentinel formula execution failed")
    factor, factor_validity = executed
    metric_validity = factor_validity & target_validity & torch.isfinite(research_target)
    ...
    cache_key = _hash_json(
        {
            ...
            "research_input_hash": _hash_tensors(research_values, research_validity, research_target, target_validity),
    ...
    diagnostic = StackVM(vocab).execute_with_validity(list(dataset.formula_tokens), diagnostic_values, diagnostic_validity)
    ...
    return {
        **core,
        "result_hash": _hash_json({key: value for key, value in core.items() if key != "cache_key"}),
        "diagnostic_hash": _hash_tensors(*diagnostic, diagnostic_target, diagnostic_available),
...
def _mutate_dataset(dataset: FirewallSentinelDataset, firewall: DateFirewall, *, after_cutoff: bool) -> FirewallSentinelDataset:
    ...
    candidates = [index for index, date in enumerate(dataset.trade_dates) if (date > firewall.research_end_date) == after_cutoff]
    if not candidates:
        raise ValueError("sentinel mutation has no candidate date")
    eligible = ResearchDataView(firewall, dataset.trade_dates).eligible_indices
    selected = candidates[0] if after_cutoff else next((index for index in reversed(candidates) if index in eligible), candidates[-1])
    dependency = next((token for token in dataset.formula_tokens if 0 <= token < values.shape[1]), None)
    ...
    if dependency is not None and dependency_mask is not None and dependency_mask.any():
        stock, feature = int(torch.nonzero(dependency_mask, as_tuple=False)[0, 0]), int(dependency)
    else:
        stock, feature = (int(value) for value in feature_positions[0].tolist())
    values[stock, feature, selected] += 1000.0
    target[int(target_positions[0, 0]), selected] += 1000.0
...
def _metric_payload(factor: torch.Tensor, target: torch.Tensor, validity: torch.Tensor) -> dict[str, float]:
    ...
    for date_index in range(factor.shape[1]):
        ...
        if int(mask.sum()) < 2:
            continue
        left, right = _rank(factor[mask, date_index]), _rank(target[mask, date_index])
        left, right = left - left.mean(), right - right.mean()
        denominator = left.norm() * right.norm()
        if float(denominator) > 1e-12:
            rank_ics.append(float((left * right).sum() / denominator))
    return {
        "coverage": float(validity.float().mean()),
        "rank_ic": float(sum(rank_ics) / len(rank_ics)) if rank_ics else 0.0,
        "valid_count": float(validity.sum()),
    }
