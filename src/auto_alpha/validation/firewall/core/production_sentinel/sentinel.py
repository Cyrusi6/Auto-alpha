...
from auto_alpha.validation.firewall.core.engineering_closure.research_view import publish_research_projection, validate_research_projection
from auto_alpha.validation.lab.campaigns.consolidate import consolidate_validation_results
...
def _run_local_worker(run: SentinelRunSpec, *, timeout: int) -> None:
    command = [sys.executable, "-m", "auto_alpha.validation.firewall.core.production_sentinel.sentinel", "worker", "--config", run.config_path]
    completed = subprocess.run(command, cwd=str(repository_root(__file__)), capture_output=True, text=True, timeout=timeout)
...
def _run_scheduler_workers(runs: Sequence[SentinelRunSpec], output_root: Path, *, timeout: int) -> dict[str, dict[str, Any]]:
    ...
    jobs = [
        ComputeJobSpec(
            ...
            command=[sys.executable, "-m", "auto_alpha.validation.firewall.core.production_sentinel.sentinel", "worker", "--config", run.config_path],
            cwd=str(repository_root(__file__)),
