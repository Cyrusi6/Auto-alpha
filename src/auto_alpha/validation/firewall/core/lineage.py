...
def _code_semantic_hash() -> str:
    modules = [
        "auto_alpha.research.formulas.runtime.vm",
        "auto_alpha.research.formulas.runtime.validity",
        "auto_alpha.research.discovery.factory.proxy_eval",
        "auto_alpha.research.discovery.factory.full_research",
        "auto_alpha.research.discovery.factory.research_policy",
        "auto_alpha.research.discovery.factory.scoring",
        "auto_alpha.research.discovery.evaluation.multi_objective",
        "auto_alpha.validation.lab.engine.metrics",
        "auto_alpha.research.formulas.batch.evaluator",
        "auto_alpha.validation.firewall.core.firewall",
    ]
    digest = hashlib.sha256()
    for module in modules:
        spec = importlib.util.find_spec(module)
        if spec is None or spec.origin is None:
            raise RuntimeError(f"research_lineage_module_unresolved:{module}")
