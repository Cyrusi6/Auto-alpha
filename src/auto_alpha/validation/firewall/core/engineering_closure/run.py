        ...
        for mutation in RUN_MUTATIONS:
            ...
            if execution=='local':
                ...
                with log.open('w') as h:
                    p=subprocess.run([sys.executable,'-m','auto_alpha.validation.firewall.core.engineering_closure.worker','--config',str(cfg)],cwd=str(repository_root(__file__)),stdout=h,stderr=subprocess.STDOUT)
