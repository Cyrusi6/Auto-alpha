...
from auto_alpha.validation.firewall.engineering_closure_audit import ALLOWED_COMPONENTS
from auto_alpha.validation.firewall.engineering_closure_audit import source_hash
from auto_alpha.validation.firewall.engineering_closure_bundle import validate_bundle
from auto_alpha.validation.firewall.engineering_closure_contracts import EVIDENCE_SCOPE
from auto_alpha.validation.firewall.engineering_closure_contracts import PROTOCOL_VERSION
from auto_alpha.validation.firewall.engineering_closure_contracts import RUN_MUTATIONS
from auto_alpha.validation.firewall.engineering_closure_contracts import RUN_PATHS
from auto_alpha.validation.firewall.engineering_closure_contracts import SENTINEL_SCHEMA
from auto_alpha.validation.firewall.engineering_closure_research_view import load_research_projection_manifest
...
def run(config_path:str|Path)->dict[str,Any]:
        ...
        for mutation in RUN_MUTATIONS:
            ...
            if execution=='local':
                ...
                with log.open('w') as h:
                    p=subprocess.run([sys.executable,'-m','auto_alpha.validation.firewall.engineering_closure_worker','--config',str(cfg)],cwd=str(repository_root(__file__)),stdout=h,stderr=subprocess.STDOUT)
                ...
                job=ComputeJobSpec(job_id=job_id,job_kind=ComputeJobKind.SHELL_COMMAND,command=[sys.executable,'-m','auto_alpha.validation.firewall.engineering_closure_worker','--config',str(cfg)],cwd=str(repository_root(__file__)),input_paths=[str(cfg),config['bundle_manifest']],output_dir=str(out),required_device_type=ComputeDeviceType.CPU,max_duration_seconds=float(config.get('timeout_seconds',3600)),max_retries=0,metadata={'task':'054-C','path':path_name,'mutation':mutation,'bundle_hash':bundle['content_hash']})
    ...
    for path_name,mutation,_,_,out in specs:
        payload=json.loads((out/'auto_alpha.execution.trading.engine.json').read_text()); executions.setdefault(mutation,{})[path_name]=payload
...
def validate_sentinel(path:str|Path,*,root:str|Path)->dict[str,Any]:
    ...
    for mutation in RUN_MUTATIONS:
      ...
      for path_name in RUN_PATHS:
        ...
        native_execution=json.loads((out/'auto_alpha.execution.trading.engine.json').read_text())
