...
from live_readiness.network_authorization.io import canonical_hash, publish_generation, sha256_file, validate_generation
...
def _source_entries(repository: Path) -> list[dict[str, Any]]:
    ...
    if not entries or not any(
        row["path"].startswith("live_readiness/production_authority/") for row in entries
    ):
