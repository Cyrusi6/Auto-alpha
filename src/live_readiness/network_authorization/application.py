...
from live_readiness.evidence_hardening.network import _validate_records
from live_readiness.evidence_hardening.transport import evidence_use_identity, transport_identity
from live_readiness.production_hardening.causal import validate_fee_aware_causal_frontier
from live_readiness.production_hardening.network_state import PLAN_SCHEMA, consolidate
from live_readiness.production_hardening.truth import validate_truth_v2
...
def _rebuild_dag(action: Mapping[str, Any], seal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if kind == "immutable_daily_raw_repair":
        stages = [
            _stage("raw_repair", "live_readiness.network_authorization.application.apply_native_canary_response", ["verified_response_rows", "immutable_raw_repair_delta"]),
            ...
            _stage("v3_tensor", "feature_factory.engineering_replay.orchestrator.build_v3_tensor_generation", ["v3_tensor_manifest", "values_sha", "validity_sha"]),
            _stage("research_firewall", "research_firewall.production_sentinel.sentinel.run_task054b_production_sentinel", ["sentinel_manifest", "research_semantic_root"]),
            ...
            _stage("fee_aware_causal_frontier", "live_readiness.production_hardening.causal.build_fee_aware_causal_frontier", ["causal_frontier_manifest", "next_frontier_root"]),
        ]
        branch = "positive_daily_raw_repair_full_lineage_rebuild"
    elif kind == "vendor_daily_absence":
        stages = [
            _stage("vendor_absence_evidence", "live_readiness.network_authorization.application.apply_native_canary_response", ["verified_empty_daily_attestation"]),
            _stage("truth_rebuild", "live_readiness.production_hardening.truth.build_truth_v2", ["truth_v2_manifest"]),
            _stage("fee_aware_causal_frontier", "live_readiness.production_hardening.causal.build_fee_aware_causal_frontier", ["causal_frontier_manifest", "next_frontier_root"]),
            _stage("dynamic_l2", "live_readiness.network_authorization.application.build_dynamic_l2_from_l1_empty", ["dynamic_l2_plan"]),
        ]
        branch = "empty_daily_vendor_absence_then_dynamic_l2"
    else:
        stages = [
            _stage("suspension_evidence_overlay", "live_readiness.network_authorization.application.apply_native_canary_response", ["suspension_evidence_overlay"]),
            _stage("truth_rebuild", "live_readiness.production_hardening.truth.build_truth_v2", ["truth_v2_manifest"]),
            _stage("fee_aware_causal_frontier", "live_readiness.production_hardening.causal.build_fee_aware_causal_frontier", ["causal_frontier_manifest", "next_frontier_root"]),
        ]
