...
from live_readiness.network_authorization.io import canonical_hash, read_json, validate_generation
from live_readiness.production_authority.application import _production_context
    ...
    for ordinal, row in enumerate(rows, start=1):
        if (
            ...
            or not str(row.get("validator_fqn") or "").startswith(
                "live_readiness.correctness_closure.application_components."
            )
        ):
