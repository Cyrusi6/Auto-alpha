...
def run_governed_backfill(config: GovernedBackfillConfig) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "schema_version": "2.0",
        "producer": "backfill_repair.governed_replay.backfill",
...
def _source_generation_code_hash() -> str:
    paths = (
        ...
        Path(__file__).resolve().parents[2] / "data_pipeline" / "ashare" / "cache.py",
        Path(__file__).resolve().parents[2] / "data_pipeline" / "ashare" / "providers" / "tushare_client.py",
        Path(__file__).resolve().parents[2] / "data_pipeline" / "ashare" / "request_normalization.py",
        Path(__file__).resolve().parents[2] / "data_pipeline" / "ashare" / "dataset_registry.py",
    )
    digest = hashlib.sha256()
    for path in paths:
        digest.update(path.relative_to(Path(__file__).resolve().parents[2]).as_posix().encode("utf-8"))
        digest.update(b"\0")
