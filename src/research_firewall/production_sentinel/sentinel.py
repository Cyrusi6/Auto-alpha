...
from research_firewall.engineering_closure.research_view import publish_research_projection, validate_research_projection
...
def _run_local_worker(run: SentinelRunSpec, *, timeout: int) -> None:
    command = [sys.executable, "-m", "research_firewall.production_sentinel.sentinel", "worker", "--config", run.config_path]
    completed = subprocess.run(command, cwd=str(Path(__file__).resolve().parents[3]), capture_output=True, text=True, timeout=timeout)
