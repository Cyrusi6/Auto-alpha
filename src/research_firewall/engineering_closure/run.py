        ...
        for mutation in RUN_MUTATIONS:
            ...
            if execution=='local':
                ...
                with log.open('w') as h:
                    p=subprocess.run([sys.executable,'-m','research_firewall.engineering_closure.worker','--config',str(cfg)],cwd=str(Path(__file__).resolve().parents[3]),stdout=h,stderr=subprocess.STDOUT)
                ...
                job=ComputeJobSpec(job_id=job_id,job_kind=ComputeJobKind.SHELL_COMMAND,command=[sys.executable,'-m','research_firewall.engineering_closure.worker','--config',str(cfg)],cwd=str(Path(__file__).resolve().parents[3]),input_paths=[str(cfg),config['bundle_manifest']],output_dir=str(out),required_device_type=ComputeDeviceType.CPU,max_duration_seconds=float(config.get('timeout_seconds',3600)),max_retries=0,metadata={'task':'054-C','path':path_name,'mutation':mutation,'bundle_hash':bundle['content_hash']})
