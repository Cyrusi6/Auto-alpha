...
`post_download_orchestrator/` creates the safe post-download sequence after a real download completes: observer refresh, landing QA, repair review, compact/validate/stats, data lake version and freeze, PIT/leakage/corporate reports, matrix refresh, artifact schema validation, freeze candidate package, and a real-data dry smoke. It now has a local execute/resume state machine with step runs, events, final package, and artifact catalog, while still defaulting to plan-only and blocking mutation when readiness is not green.
