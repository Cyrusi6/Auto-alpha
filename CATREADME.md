...
`production_replay/` runs the day-level orchestrator across a date window. It writes replay plans, day summaries, events, a package, and a replay report while preserving resume state. `shadow_lab/` aggregates multi-day shadow artifacts into performance, drift, and calibration-suggestion reports. `live_readiness/` converts replay, shadow lab, certification, freeze, incident, monitoring, settlement, and reconciliation artifacts into a policy-driven scorecard and readiness decision.
