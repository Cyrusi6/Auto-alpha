...
共 25 个可见子系统、57 个 Python package 目录。`research`、`validation`、`portfolio`、`execution` 在子系统下不再继续套包，runner/store/report 通过同级能力前缀区分。结构审计将 package 上限锁定为 65。测试目录按相同六领域镜像。任务编号不再是包名；Task 055 只保留最终、扁平的 `platform/network_authority`，旧代源码、`_internal` 历史树、公开入口和测试均已删除。
