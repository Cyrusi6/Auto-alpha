...
def aggregate_replay_days(day_results: list[ProductionReplayDayResult | dict[str, Any]]) -> dict[str, Any]:
    ...
    broker_connectivity_success = sum(1 for row in rows if row.get("broker_connectivity_status") in {"passed", "warning"})
    readonly_mirror_success = sum(1 for row in rows if row.get("broker_readonly_mirror_status") in {"success", "warning"})
    readonly_mirror_break_count = sum(int(row.get("readonly_mirror_break_count") or 0) for row in rows)
    ...
    return {
        ...
        "broker_connectivity_success_day_count": broker_connectivity_success,
        "readonly_mirror_success_day_count": readonly_mirror_success,
        "readonly_mirror_break_count": readonly_mirror_break_count,
