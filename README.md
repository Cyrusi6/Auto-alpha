...
The release candidate uses a non-circular implementation (`I`) → evidence-only (`E`) → final anchor (`A`) topology. Final implementation `I` is `96f25cf5d637a3eb16461b0e52b691b506f6a0e8`; source root is `1c716160e267909e8896e44cc12e497ae5a45b22d50844f498f6602e2ddc89a1`. The authoritative standard-library verifier must be extracted from exact `I` and receive the exact `A` object ID plus an anchor digest fixed outside the candidate checkout. Candidate self-checks do not constitute independent review or network authorization.

The final offline production-shaped rehearsal is `e7f3673a807c601bad9801b75c4f9e36bb3e086e963bcc7e8eed5af3e5423e3f`: positive/empty primary and independent sibling chains each execute 12 stages, primary resume reuses all 12 stages, and all 47 crash/recovery cases pass. Each branch independently recomputes 100 net and 100 all-in terminal pairs, all truthfully remaining `causal_valuation_blocked`. Candidate evidence is `evidence/task_055_k/task055kr2_candidate_evidence.json`; it is non-executable, network-unauthorized, and only ready for independent audit.
