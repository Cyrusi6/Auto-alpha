...
Task 055-KR2 supersedes the prior READY evidence without changing historical files. Gateway and 12-stage application publication now use a descriptor-first lease that retains the parent and lease descriptors, binds an unpredictable nonce and monotonic durable fence, verifies xattr/sealed bytes/canonical parent at every side-effect boundary, and prevents a fenced owner from committing or deleting a successor lease.

Release trust is frozen as implementation (`I`) → evidence-only (`E`) → final anchor (`A`). The authoritative verifier must be extracted from exact `I` and receive the exact `A` object ID plus an anchor digest fixed outside the candidate checkout. It never executes target Python code and rejects dirty trees, HEAD movement, new commits, blob drift, self-supplied anchors, and fully rehashed semantic chains.

The offline production-shaped positive/empty rehearsal executes primary and independent sibling 12-stage chains, reuses all 12 primary stages on immutable resume, and covers 47 crash boundaries. Both net and all-in 100-run sets remain truthfully `causal_valuation_blocked`. Candidate evidence is `evidence/task_055_k/task055kr2_candidate_evidence.json`; it is non-executable, network-unauthorized, and only ready for an independent audit. A candidate self-check is not that independent review.
