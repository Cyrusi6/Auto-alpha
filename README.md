...
### Task 055-K single-canary correctness closure

Task 055-K separates the normalized request fingerprint, transport identity, and evidence-use identity throughout capability issuance, envelope validation, signed broker receipts, cache acceptance, and response application. The fixed canary remains `daily / 000413.SZ / 20160726`; the release is offline and does not expose an executable token or network command.

The sole production transport boundary is the Task 055-K broker. Legacy Task 052/055-C/D/F/G/H/I/J network entrypoints fail closed before credential, TLS, or HTTP access. Rehearsal transport replaces only the lowest response-byte boundary, while retaining the production serializer, parser, capability, receipt signature, cache, staged application, 12-path Research Firewall sentinel, exact-20 materialization, and Fee-aware 20×5 causal replay.

The real-context positive and empty rehearsals each produced 100 net and 100 all-in terminal rows, independently recomputed primary/sibling roots, and immutable primary resume. Current terminal rows remain `causal_valuation_blocked`; this is a truthful engineering frontier, not simulator completion or Alpha evidence. The top state is `task055k_single_canary_engineering_ready_waiting_operator_authorization_no_network_executed`, with `network_authorized=false` and `operational_state_unproven=true`.

Git-safe evidence is `evidence/task_055_k/task055k_scrubbed_evidence.json`. Its source root uses Git blob identity plus index mode (`100644`/`100755`) and is portable across clean clones. Task 055-J evidence is intentionally marked non-portable because it encoded server filesystem permission bits. Verify Task 055-K with `python task_055_k/verifier.py evidence/task_055_k/task055k_scrubbed_evidence.json --repository-root .`; this does not authorize a real request.

### Task 055-KR corrective single-canary closure

Task 055-KR supersedes the preceding Task 055-K READY evidence without rewriting it. `evidence/task_055_k/task055k_scrubbed_evidence.json` remains historical, but is now `superseded=true`, `executable=false`, and `authorization_eligible=false` through the new immutable supersession lineage. The executable boundary is re-established from implementation commit `21fb2f78a694d1876b74c156b96c8c91c98d6e82` and source root `a4d958419bec8160cda2741cbbbc6782e176cc83e9e208c4f65a18616a9bad35`.

The final HTTPS call is reachable only through the Task 055-KR gateway. It revalidates the canonical authority, checkpoint, reviewed candidate seal, operator authorization, ordered 17-key plan, fixed first canary, three request identities, reservation, public key, journals, single-flight lock, TLS result, and credential budget immediately before the one POST. All Task 052 and Task 055-C/D/F/G/H/I/J legacy network functions fail closed before credential or transport access; the generic Tushare client is transport-disabled.

Positive and empty production-shaped rehearsals each executed 12 stages in primary and independent sibling roots, then reused all 12 immutable stages on resume. Both branches independently recomputed 100 net and 100 all-in terminal pairs; all remain `causal_valuation_blocked`. The crash matrix covered 37 generic stage boundaries plus 10 real production-component boundaries, including Sentinel, net/all-in replay, and final-pointer recovery.

The published state is `task055k_single_canary_engineering_ready_waiting_operator_authorization_no_network_executed`. Candidate authority, checkpoint, final candidate seal, native rehearsal, and final report roots are `9960eafb...5981`, `9fded99c...e6d4`, `0b371a92...062e`, `679754fc...ed76`, and `ab691932...a6f4`. Credential reads, Tushare POSTs, other HTTP requests, GPU jobs, and prospective-holdout accesses are all zero; maximum read date is `2026-06-30`. `network_authorized=false`, `authorization_eligible=false`, and `operational_state_unproven=true` remain explicit.

Git-safe evidence is `evidence/task_055_k/task055kr_scrubbed_evidence.json`. Verify it with `python task_055_k/verifier.py evidence/task_055_k/task055kr_scrubbed_evidence.json --repository-root .`; the verifier is standard-library only and does not authorize a request. The 31-case recomputed-self-hash mutation matrix and a clean-clone tamper check both fail closed as expected.

### Task 055-KR2 replacement-safe lease and external release anchor

Task 055-KR2 replaces pathname/inode-only locking with a descriptor-first lease that retains canonical-parent and lease descriptors, binds an unpredictable nonce, sealed bytes, xattr identity, and a monotonic durable fence, and revalidates ownership at every credential, transport, receipt, stage-commit, pointer-publication, and owner-safe-release boundary. Same-path replacement, inode reuse, truncate/rewrite, parent replacement, ABA, stale-owner resurrection, unsafe cleanup, and completed-fast-path replacement all fail closed.

The release candidate uses a non-circular implementation (`I`) → evidence-only (`E`) → final anchor (`A`) topology. Final implementation `I` is `276b02fc64bd5e71a7cbfa4866fdbdf11794f378`; source root is `5a78778f930e7cf09b701cda4a08e3ce9f1789ca9b6531883b7ebbbbd5e27cab`. The authoritative standard-library verifier must be extracted from exact `I` and receive the exact `A` object ID plus an anchor digest fixed outside the candidate checkout. Candidate self-checks do not constitute independent review or network authorization.

The final offline production-shaped rehearsal is `de898d13a679e8ea18344d2131f2efe04176a5413429e9a189ab2829f2528ff9`: positive/empty primary and independent sibling chains each execute 12 stages, primary resume reuses all 12 stages, and all 47 crash/recovery cases pass. Each branch independently recomputes 100 net and 100 all-in terminal pairs, all truthfully remaining `causal_valuation_blocked`. Candidate evidence is `evidence/task_055_k/task055kr2_candidate_evidence.json`; it is non-executable, network-unauthorized, and only ready for independent audit.
